...
function usernameCheck(){	 
	 ...
	 if(username==""){
		 document.all.deny.innerHTML="用户名不能为空";
		 buttonDisEnabled();
	 }
	 else{
		var request = createXmlHttpRequest();//创建request的对象
		request.open("post","registerCheck.html?username="+username);
	 	request.send();
		 ...
		 var value = request.responseText;
	 	 if(value=="true"){
	 		document.all.access.innerHTML="";
	 		 document.all.deny.innerHTML="该用户名已经被注册";
	 		buttonDisEnabled();
	 	 	}
	 	}
		 ...
		 document.all.access.innerHTML="该用户可以注册";
		 buttonEnabled();
	 	}
     }
	 }
   ...
   function buttonEnabled(){
	   if(document.all.access_email.innerHTML!=""&&document.all.access_password.innerHTML!=""&&document.all.access.innerHTML!=""&&document.all.access_password_right.innerHTML!=""&&document.all.access_identify.innerHTML!="")
	   document.all.submitcheck.disabled = false;
   }
   function emailCheck(){	 
		 var email = document.all.email.value;//获得text的值
		 if(email==""){
			 document.all.deny_email.innerHTML="邮箱不能为空";
			 buttonDisEnabled();
		 }
			 ...
			 var pattern = /^([\.a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+(\.[a-zA-Z0-9_-])+/;  
			    if (!pattern.test(email)) {  
			    	 document.all.access_email.innerHTML="";
			    	 document.all.deny_email.innerHTML="请输入正确的邮箱格式";
			    	 buttonDisEnabled();
			    }  
		 
			    else{
			    	var request = createXmlHttpRequest();//创建request的对象
			    	request.open("post","emailCheck.html?email="+email);
			    	request.send();
			    	request.onreadystatechange = function(){
			    		if(request.readyState==4&request.status==200)
			    		{
			    			...
			    			if(value=="true"){
			    				 document.all.access_email.innerHTML="";
			    				 document.all.deny_email.innerHTML="该邮箱已经被注册";
			    				 buttonDisEnabled();
			    				
			    			}
			    		}
			    		else{
			    			document.all.deny_email.innerHTML="";
			    			document.all.access_email.innerHTML="该邮箱可以注册";
			    			 buttonEnabled();
			    		}
			    	}
			    }
